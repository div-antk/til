...
- VPCを設置しないでEC2インスタンスを立ち上げる場合、VPCはどのような設定になるか
  - 単一のデフォルトVPCにパブリックサブネットが1つデフォルトで設定される

- ネットワークACLの特徴は
  - ✗: VPC全体のトラフィックをコントロールする
  - ✗: インスタンスのトラフィックをコントロールする
  - ✗: 通常はインバウンドの許可設定のみを行う
  - ステートレスである（セキュリティグループとは異なり、ネットワークACLはステートレスなため、返されたトラフィックをルールによって明示的に許可する必要がある。セキュリティグループはステートフルである）

- Direct Counnectの特徴として間違っているものは
  - ✗: ネットワーク信頼性/帯域幅が高い
    - Direct Connectを使用すると、専用線接続を利用するデータと、そのデータがルーティングされる方法を選択できるため、インターネットベースの接続において、より安定性のあるネットワーク体験を提供できる
  - Direct Connectデバイスを自社データセンターに設置して接続

- VPCエンドポイントのゲートウェイ型の特徴は
  - VPC内部から直接外のサービスと通信する

- VPC peeringの特徴として間違っているものは
  - ✗: 接続したいVPCとは直接Peeringする必要がある
  - VPCであればすべてのリージョン間で異なるVPC間のピア接続も可能
    - 中国リージョンは政治的な理由でできない

- VPC Flow logsの特徴について間違っているものは
  - セキュリティグループだけでaccepted/rejectされたトラフィックログを取得する
