...
1. 信頼性のセクションで作成したアーキテクチャにRDSとNATゲートウェイを設置し直す
2. シンガポールリージョンにセカンダリーの構成を構築する
3. セカンダリーへとフェイルオーバー設定を実施する
4. 東京リージョンを停止してフェイルオーバー結果を確認する

- 時間がかかるのでRDSから作る
- NATゲートウェイを作る
- NATゲートウェイにルートテーブルを設定する
  - 『ルートテーブル』の画面だとどれがパブリックに紐付いているルートテーブルかわかりづらいので、サブネットの一覧から飛んだほうがわかりやすい
  - 『ルート』のタブから『ルートの編集』、ターゲットに「NAT」を指定してフルオープン（ `0.0.0.0/0` ）
- RDSを別リージョンにコピーする。EC2はAMIを使ってコピーしたが、RDSはSnapshotを使ってコピーする
  - アクションから『スナップショットの取得』本来は停止させてからやったほうがいいけど今回は使用してないのでそのままやる
- アクションからスナップショットのコピー
  - シンガポールリージョン、識別子をつけて完了
- シンガポールリージョンに移動
- VPCを作成
  - 『パブリックとプライベートサブネットを持つVPC』
    - 『Elastic IP 割り当て ID』で表示されないことがある。そのときは別タブでVPCコンソールを開いて『NATゲートウェイの作成』 -> 『新しいEIPの作成』を押して表示された値をコピーして貼り付けると使える
- プライベートとパブリックのサブネットを1cで作成
- インターネットゲートウェイと繋がっているルートテーブルを選択して『サブネットの編集』で `10.0.2.0/24` を追加選択して決定
- NATと繋がっているルートテーブルを選択して『サブネットの編集』で `10.0.n.0/24` が奇数のものを選択して決定
- サブネット4つを『アクション』からIPの自動割り当てを有効にする
- EC2のAMIからパブリック1aと1cのインスタンスを作る
- RDSに行ってスナップショットの復元
  - 設定は東京のものとほぼ同じ。セキュリティグループは新規
- 作ったDBのセキュリティグループは上で作った新規のものなので再設定をする必要がある。DBの『セキュリティグループ』をクリック
  - 『インバウンドルール』からフルオープンのものに編集する
- ターミナルからEC2に入ってそれぞれのインスタンスがわかるようにhtmlを編集
- ロードバランサーを作成。ALB
  - パブリックのサブネットをそれぞれ選択
  - セキュリティグループは新規でHTTPのみ
  - ルーティングの設定で名前
  - ターゲットの登録で2つのEC2を選択
- オードスケーリンググループを作成する
- まず起動設定を作る
  - amiから作る
  - 作り終えたらそのままAuto Scalingグループの作成
    - グループサイズは2、vpcを選択してパブリックのサブネットを2つ選択。次へ
    - 『スケーリングポリシーを使用して、このグループのキャパシティを調整する』
    - スケーリング範囲は2〜4
    - 増加の『アラームの追加』で通知は切る、CPU使用率50以上
    - 追加2
    - 増加の『アラームの追加』で通知は切る、CPU使用率 <=15
    - 削除2
    - 作成後、設定忘れてたのがあるので編集。『ターゲットグループ』をalb、『終了ポリシー』をNewest
- 1aと1cのインスタンスを『アクション』から既存のオートスケーリンググループをアタッチ
- これでシンガポールの構成は完了したので東京リージョンと繋いでフェイルオーバー構成をしていく
- Route53からレコードセットの作成
  - エイリアスを「はい」にして東京リージョンにあるELBを選択
  - ルーティングポリシーで「フェイルオーバー」
  - プライマリ
  - 『ターゲットの正常性の評価』は「はい」
    - ロードバランサーのヘルスチェックを使うかどうかという設定
  - シンガポールのぶんも作成。シンガポールのELB、セカンダリを選択して作成
- 東京リージョンのオードスケーリンググループを削除
