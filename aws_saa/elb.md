...
- 大規模アクセスが予測される際にCLBやALBで必須だったPre-warming（暖機運転）申請が不要
...
- イメージから作った場合は設定がさっきのと同じなのでセキュリティグループまでパス
  - これうまくいかなかったので結局イメージからではなく新規で作成
- 同じ手順でインスタンスを作成し、 `index.html` を `1c` で書き換えた

## ELBによる冗長構成ハンズオンのメモ（ELBによる分岐）

- EC2メニュー左の『ロードバランサー』から作成
- ALBを選択
- プロトコルはHTTPのみ
  - よりセキュアな構成にしたい場合はHTTPSを追加して設定する
- AZの選択で、AZ 1aにはpublic-1a、AZ 1cにはpublic-1cをそれぞれ選択
- 『セキュリティグループ』はインスタンスで選択したものと同じものを選択
- 『ルーティングの設定』は名前をつけて『ターゲット』はインスタンスを選択
  - 『ヘルスチェック』で正常、非正常のしきい値をテストのために3ずつ、間隔も10秒
- 『ターゲットの登録』で2つのAZのインスタンスを選択して登録済みに追加
- 作成して完了
- 作ってしばらくしたらELBのDNS名をコピー。ブラウザに貼り付けるとそれぞれのインスタンスに飛ぶ。今回の場合更新するたびに1aと1cが交互に表示される。バランスをとっていることがわかる

### 負荷分散のテスト

- インスタンスを1つ停止する
- ELBのターゲットの確認で状態が確認できる
- 上記のDNSのURLを見てみると今度は片方しか表示されない