...
- ターミナルから作られたインスタンスに入ってそれぞれのhtmlファイルを別内容でいじる
- またロードバランサーのDNSのURLにアクセス。更新するたびオートスケーリングのhtmlの内容が表示されるため、負荷分散されてることがわかる
- しかしこれだと実際のところ設計とは違う
  - 2つインスタンスがあって、負荷がかかると4つに増えるという設定にするはずだったが、最初から4つになってしまっている
  - 今の設定だと2つの追加されたインスタンスのヘルスチェックをしてインスタンスが増えるという設定になってしまっている
  - インスタンスの『アクション』からAuto-Scalingグループをアタッチすればいいが、現在はターミネーションポリシーが「2つを維持する」という設定になっているため、アタッチした瞬間に削除されてしまう。なのでターミネーションポリシーを変更する必要がある
...
- しばらくすると2つのインスタンスがスケールアウトする
- 負荷を下げると新しく作られたインスタンスがターミネートされるはず

```bash
pkill stress
```

- Auto-Scalingグループのコンソールを見ると4だった希望数が2に変わっている（なってないこともあるっぽい）。それに合わせてインスタンスがターミネート（スケールイン）しているのがわかる
